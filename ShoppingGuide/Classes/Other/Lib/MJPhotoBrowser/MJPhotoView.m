...
- (void)handleLongPress:(UILongPressGestureRecognizer *)tap{
    if (tap.state == UIGestureRecognizerStateBegan) {
        ...
        //http://www.jianshu.com/p/24255cd669b1
        ...
        UIAlertAction *archiveAction = [UIAlertAction actionWithTitle:@"保存" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            // 通知代理
            if ([self.photoViewDelegate respondsToSelector:@selector(photoViewLongPress:)]) {
                [self.photoViewDelegate photoViewLongPress:self];
            }
        }];
